...
# Runs on the local Mac with USB iPhone, Xcode and WebDriverAgent.
...
ensure_pymobiledevice3_tunneld() {
  ...
  if [ "${start_status}" -ne 0 ]; then
    ...
    if [ "${start_status}" -ne 0 ]; then
      log_pymobiledevice3_tunneld_failure "pymobiledevice3 tunneld 自动启动失败，日志: ${tunneld_log}。iOS 17+ CPU/内存采样依赖该 root 常驻服务，请先执行 scripts/ios/install-pymobiledevice3-tunneld-launchdaemon.sh 做一次性配置。"
      return 1
    fi
  fi
...
find_wda_project() {
  ...
  # Existing local WDA installations remain usable without their former device service.
  local candidates=(
...
ensure_wda_ready() {
  ...
  if [ -z "${wda_project}" ]; then
    ...
    if [ -z "${wda_project}" ]; then
      ...
      WDA_READY_ERROR="未找到 WebDriverAgent.xcodeproj，无法自动启动 WDA。请先执行 sh scripts/ios/setup-monkey-runtime.sh，或配置 WDA_PROJECT_PATH。"
      return 1
    fi
  fi
