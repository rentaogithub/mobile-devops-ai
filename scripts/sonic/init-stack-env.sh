...
# Initialize deploy/sonic/.env for the current build Mac.
...
SONIC_HOST="${SONIC_HOST:-$(detect_local_ipv4)}"
...
SONIC_EUREKA_IMAGE="${SONIC_EUREKA_IMAGE:-sonicorg/sonic-server-eureka:v2.7.2}"
SONIC_GATEWAY_IMAGE="${SONIC_GATEWAY_IMAGE:-sonicorg/sonic-server-gateway:v2.7.2}"
SONIC_CONTROLLER_IMAGE="${SONIC_CONTROLLER_IMAGE:-sonicorg/sonic-server-controller:v2.7.2}"
SONIC_FOLDER_IMAGE="${SONIC_FOLDER_IMAGE:-sonicorg/sonic-server-folder:v2.7.2}"
...
SONIC_EUREKA_IMAGE=${SONIC_EUREKA_IMAGE}
SONIC_GATEWAY_IMAGE=${SONIC_GATEWAY_IMAGE}
SONIC_CONTROLLER_IMAGE=${SONIC_CONTROLLER_IMAGE}
SONIC_FOLDER_IMAGE=${SONIC_FOLDER_IMAGE}
SONIC_EUREKA_USERNAME=sonic
SONIC_EUREKA_PASSWORD=$(random_secret)
SONIC_SECRET_KEY=$(random_secret)
SONIC_AGENT_SERVER_PORT=${SONIC_WEB_PORT}
...
EOF
  ...
  replace_or_append SONIC_EUREKA_IMAGE "$SONIC_EUREKA_IMAGE" "$SONIC_ENV_FILE"
  replace_or_append SONIC_GATEWAY_IMAGE "$SONIC_GATEWAY_IMAGE" "$SONIC_ENV_FILE"
  replace_or_append SONIC_CONTROLLER_IMAGE "$SONIC_CONTROLLER_IMAGE" "$SONIC_ENV_FILE"
  replace_or_append SONIC_FOLDER_IMAGE "$SONIC_FOLDER_IMAGE" "$SONIC_ENV_FILE"
  replace_or_append SONIC_EUREKA_USERNAME "sonic" "$SONIC_ENV_FILE"
  replace_or_append SONIC_AGENT_SERVER_PORT "$SONIC_WEB_PORT" "$SONIC_ENV_FILE"
  if ! grep -qE '^SONIC_EUREKA_PASSWORD=.{16,}$' "$SONIC_ENV_FILE"; then
    replace_or_append SONIC_EUREKA_PASSWORD "$(random_secret)" "$SONIC_ENV_FILE"
  fi
  if ! grep -qE '^SONIC_SECRET_KEY=.{16,}$' "$SONIC_ENV_FILE"; then
    replace_or_append SONIC_SECRET_KEY "$(random_secret)" "$SONIC_ENV_FILE"
  fi
  echo "Updated Sonic env: $SONIC_ENV_FILE"
fi

if [ -f "$SONIC_ENV_FILE.bak" ]; then
  unlink "$SONIC_ENV_FILE.bak"
fi
echo "SONIC_HOST=${SONIC_HOST}"
echo "SONIC_WEB_IMAGE=${SONIC_WEB_IMAGE}"
echo "SONIC_SERVER_VERSION=2.7.2"
