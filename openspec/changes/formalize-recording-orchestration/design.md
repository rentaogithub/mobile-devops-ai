...
本变更提出时，录制数据保存在文件系统，流程模板按请求临时生成，可视化画布以内存 DSL 工作。后端已经具有 DSL 校验、编译、执行和运行证据持久化，因此本变更将资产生命周期作为独立层加入，而不重写执行引擎。

设计约束与分阶段验收（当前实现状态见平台架构和功能文档，下文为本变更的目标设计）：

- [统一设计约束与分阶段验收](../../../docs/设计与演进.md#replay-design)
