...
- 研发附加授权：管理员进入“配置管理 → 用户角色 → 编辑用户”，在目标产品线的研发角色旁勾选“允许发布到应用商店”。只对该用户和该产品线生效，不扩展到其他研发成员或其他产品线。
...
授权后，CI/CD 提供苹果商店渠道，并允许对应 AppStore 发布、提交审核、取消审核和发布状态前置检查；AI 受控发布也使用相同授权。仍需启用当前应用的 Jenkins 服务，满足 iOS 平台限制、质量门禁、发布分支、验证密码及原有确认流程。附加授权不包含平台管理或管理员门禁豁免。Android 商店执行器尚未接入。

产品运营和管理员保留原有发布能力。已有研发、新建研发和自助注册获批的研发默认没有此附加授权。成员配置 API 的 `productLines` 项支持布尔字段 `appStoreRelease`；显式 `false` 撤销，省略字段时保留同一研发成员原有值。移除产品线、停用账号或变更为非研发角色后，研发附加授权不再生效。

页面按当前产品线显示渠道；服务端和 AI 在执行时读取数据库中的最新成员授权，不能用旧页面、旧用户对象或待执行审批绕过撤销。已提交至 Jenkins / Apple 的任务不因撤销授权自动取消。授权与更新账号信息在事务内保存，非法授权不会部分修改账号。
