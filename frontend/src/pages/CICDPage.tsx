...
export default function CICDPage() {
  ...
  const renderPackageSizeAnalysis = () => {
    const largestEntry = packageSizeAnalysis?.entries?.[0];
    const comparison = packageSizeAnalysis?.comparison || null;
    const comparisonEntryMap = new Map((comparison?.entries || []).map((item) => [item.key, item]));
    if (packageSizeLoading) {
      return (
        <Card size="small">
          <Space direction="vertical" size={12} style={{ width: '100%' }}>
    ...
    if (packageSizeError) {
      return <Alert type="error" showIcon message={packageSizeError} />;
    }
    if (!packageSizeAnalysis) {
      return <Empty description="暂无包体积分析" />;
    }

    return (
      <Space direction="vertical" size={12} style={{ width: '100%' }}>
        {packageSizeAnalysis.warnings?.map((item) => (
          <Alert key={item} type="warning" showIcon message={item} />
        ))}
        <Space wrap>
          {packageSizeAnalysis.updatedAt && <Tag color="blue">存档 {new Date(packageSizeAnalysis.updatedAt).toLocaleString('zh-CN')}</Tag>}
          {packageSizeAnalysis.cached && <Tag>已存档</Tag>}
          {comparison?.baseline && (
            <Tag color="purple">
          ...
          <Button
            ...
            onClick={() => void ensurePackageSizeAnalysis(true)}
          >
            重新分析
          </Button>
        </Space>
        <Row gutter={12}>
          <Col xs={24} md={8}>
            <Card size="small">
              ...
              <div style={{ marginTop: 6, fontSize: 22, fontWeight: 600 }}>{packageSizeAnalysis.totalText || '-'}</div>
              {comparison && (
          ...
          <Col xs={24} md={8}>
            <Card size="small">
              ...
              <div style={{ marginTop: 6, fontSize: 22, fontWeight: 600 }}>{packageSizeAnalysis.uncompressedText || '-'}</div>
              {comparison && (
        ...
        <Space wrap>
          {packageSizeAnalysis.publishChannel && <Tag>{packageSizeAnalysis.publishChannel}</Tag>}
          {packageSizeAnalysis.appVersion && <Tag color="purple">APP {packageSizeAnalysis.appVersion}</Tag>}
          {packageSizeAnalysis.channelBuildNumber && <Tag color="green">渠道构建号 {packageSizeAnalysis.channelBuildNumber}</Tag>}
        </Space>
        {packageSizeAnalysis.entries.length ? (
          <Table
            size="small"
            rowKey={(record) => record.key}
            pagination={false}
            scroll={{ y: 410 }}
            dataSource={packageSizeAnalysis.entries}
            columns={[
              {
                title: '类型',
                dataIndex: 'type',
        ...
        <Space direction="vertical" size={12} style={{ width: '100%' }}>
          ...
          <Tabs
            ...
            onChange={(key) => {
              ...
              if (key === 'package-size') void ensurePackageSizeAnalysis();
            }}
            items={[
              {
                key: 'build-log',
              ...
              },
              {
                ...
                children: renderPackageSizeAnalysis(),
              },
              {
                key: 'third-sdk',
                label: (
