import { Button, Card, Col, List, message, Row, Space, Table, Tag, Typography } from 'antd';
import {
  BranchesOutlined,
  CodeOutlined,
  CopyOutlined,
...
const toolSections: ToolSection[] = [
  {
    ...
    name: 'podx 工具链',
    icon: <ToolOutlined />,
    tags: ['CocoaPods', '组件发布', '版本维护'],
    description: '用于 NN iOS 组件发布、升级、校验和本地工具链维护。',
    commands: [
      {
        title: '查看 podx 帮助',
        command: 'podx help',
    ...
    allCommands: [
      ...
      {
        title: '切回远端',
        command: 'podx disable <PodName>',
        description: '将指定组件切回远端依赖；不传组件名表示全部切回远端。',
        category: '依赖',
      },
      {
        title: '安装依赖',
  ...
  },
  {
    ...
    tags: ['多仓库', '批量操作', '代码同步'],
    description: '用于多仓库工作区的批量拉取、状态查看、分支切换和差异检查。',
    commands: [
      {
        title: '查看 mgit 帮助',
        command: 'mgit help',
...
const allToolCommands = toolSections.flatMap((section) => (
  section.allCommands.map((command, index) => ({
    ...command,
    tool: section.key,
    toolRowSpan: index === 0 ? section.allCommands.length : 0,
  }))
));
  ...
  return (
    <div>
      ...
      <Paragraph type="secondary" style={{ marginBottom: 24 }}>
        汇总日常 DevOps 工具链用法，当前先维护 podx、mgit 的常用操作。
      </Paragraph>
        ...
        {toolSections.map((section) => (
          <Col xs={24} lg={12} key={section.key}>
            ...
            >
              ...
              {renderCommandList(section.commands)}
            </Card>
          </Col>
        ))}
      </Row>
