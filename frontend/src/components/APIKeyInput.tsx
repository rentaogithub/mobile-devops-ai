...
const APIKeyInput: React.FC<APIKeyInputProps> = ({ value, onChange, onValidate }) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // 保存到 localStorage
    if (newValue.trim()) {
      localStorage.setItem('qwen_api_key', newValue);
    } else {
  ...
  return (
    <Space.Compact style={{ width: '100%' }}>
      <Input
        ...
        placeholder={isAdmin ? "输入 Codex/OpenAI API Key（可选，用于 AI 智能分析）" : "API Key 未配置（请联系管理员）"}
        disabled={!isAdmin && !value}
        prefix={
          <Tooltip title={isAdmin ? "通义千问 API Key 用于智能分析崩溃日志。保存在浏览器本地。" : "仅管理员可以配置 API Key"}>
